...
export default function Home() {
  ...
  const profileOptions = [
    {
      title: "Vendor Registration/Company details",
      status:
        vendorRegistrationData?.status === 1 ? (
          <svg
      ...
      last_updated: formattedDate(new Date(vendorRegistrationData?.updatedAt)),
      edit_link: "/vendor-profile/vendor-registration/edit",
      view_link: "/vendor-profile/vendor-registration",
    },
    {
      title: "Bank details",
      status:
        BankDetailsData?.status === 1 ? (
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
      ...
      last_updated: formattedDate(new Date(BankDetailsData?.updatedAt)),
      view_link: "/vendor-profile/bank-details",
    },
    {
      ...
      last_updated: "",
      edit_link: "",
      view_link: "/vendor-profile/agency-credit-application",
    },
    {
      title: "NDA",
      status:
      ...
      last_updated: formattedDate(new Date(NDAData?.updatedAt)),
      edit_link: "",
      view_link: "/vendor-profile/nda",
    },
  ];
  useEffect(() => {
  ...
  const showSession = () => {
    if (status === "authenticated") {
      return (
        <div className="min-h-screen rounded-sm border text-black border-stroke bg-white/80 bg-blend-screen bg-[url('/images/pexels-tirachard-kumtanom-112571-733852.jpg')] bg-cover px-6 py-6 shadow-default dark:border-strokedark sm:px-7.5">
          <LogoCard />
          <h2 className="font-medium text-xl py-3">Vendor Profile</h2>
          <div className="flex">
            {error && <span className="text-red mr-4">{error}</span>}
          </div>
          <div className="rounded-md border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark dark:text-white sm:px-7.5 xl:pb-1">
            <div className="max-w-full overflow-x-auto">
              <table className="w-full table-auto">
                <thead>
                  <tr className="bg-gray-2 text-left dark:bg-meta-4">
                    <th className="min-w-[220px] p-3 font-medium text-black dark:text-white xl:pl-11">
                      Title
                    </th>
                    <th className="min-w-[150px] p-3 font-medium text-black dark:text-white">
                      Last updated
                    </th>
                    <th className="min-w-[120px] text-end p-3 font-medium text-black dark:text-white">
                      Status
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {profileOptions?.map((item, key) => (
                    <tr key={key}>
                      <td className="border-b border-[#eee] p-3 dark:border-strokedark">
                        <Link href={item.view_link}>
                          <h2 className="font-medium text-md py-3 hover:underline duration-500 ease-in-out">
                            {item.title}
                          </h2>
                        </Link>
                      </td>
                      ...
                      <td className="border-b border-[#eee] px-4 text-end py-5 dark:border-strokedark">
                        {true ? (
